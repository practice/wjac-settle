					...
					<c:choose>
						<c:when test="${file.filetype == '1'}">
							<div><a href="download?organId=${organ.organId}&attId=${file.id}">${file.filename}</a> (${file.filesize} bytes)
								&nbsp;&nbsp; <sec:authorize ifAllGranted="ROLE_ADMIN"><a href="deleteNonApproval2Attachment?attId=${file.id}&organId=${organ.organId}"><img src="<c:url value="/img/btn_b_commt_del.gif" />"></a></sec:authorize></div>
